...
**Error Responses:**
- **Code:** 500
  - **WithdrawCoinInactive:**
    ```json
    { "error": "Receiving coin inactive" }
    ```
  - **DepositCoinInactive:**
    ```json
    { "error": "Deposit coin inactive" }
    ```
...
Returns the current Bitcoin network withdrawal fee rates in sats per byte. Fees are sourced from [https://bitcoinfees.earn.com/](https://bitcoinfees.earn.com/).
...
| Name    | Type   | Description               | Required |
|---------|--------|---------------------------|----------|
| address | String | The address to search for | Yes      |
...
| Name  | Type   | Description       | Required |
|-------|--------|-------------------|----------|
| trade | String | Uuid for trade    | Yes      |
...
|--------------------|---------|------------------------------------------------|----------|----------|
| fromCoin           | String  | Deposit coin                                   | Yes      |          |
| toCoin             | String  | Withdraw coin                                  | Yes      |          |
| withdrawAddress    | String  | Address to send coins to                      | Yes      |          |
| amount             | Amount  | Receiving coin amount to get quote for        | No       |          |
| withdrawRate       | Number  | Custom withdraw rate for BTC                  | No       |          |
| withdrawSpeed      | String  | Custom withdraw rate for BTC                  | No       | fastest  |
| emergencyAddress   | String  | Emergency address to return coins if trade fails | No     |          |
| reuse              | Boolean | Creates a reusable deposit address             | No       |          |
...
- **Code:** 500
  ...
  - **InvalidEmergencyAddress:**
    ```json
    { "error": "Invalid Emergency Address" }
    ```
  - **WithdrawRateOnlyBtc:**
    ```json
    { "error": "Withdraw rate only applys to bitcoin withdraws" }
    ```
  - **WithdrawFeeTooHigh:**
    ```json
    { "error": "BTC withdraw fee rate too high" }
    ```
  - **WithdrawFeeToLow:**
    ```json
    { "error": "BTC withdraw fee too low" }
    ```
  - **DepositCoinInactive:**
    ```json
    { "error": "Deposit coin inactive" }
  ...
  - **OpenTradeFailed:**
    ```json
    { "error": "Open trade failed" }
    ```
  - **NoWithdrawAddress:**
    ```json
    { "error": "No withdraw address submitted" }
    ```
  - **NoDepositCoin:**
    ```json
    { "error": "No deposit coin submitted" }
    ```
  - **NoWithdrawCoin:**
    ```json
    { "error": "No withdraw coin submitted" }
    ```
